...
class PlateSurveyXML(BaseXmlModel, tag="platesurvey"):
    ...
    def read_xml(cls, path: os.PathLike | str) -> Self:
        ...
        return PlateSurveyXML.from_xml_tree(
            ET.parse(path, parser=ET.XMLParser()).getroot()
        )
...
class Survey:
    ...
    def from_file(cls, path: os.PathLike | str) -> "Survey":
        ...
        xml_data = ET.parse(path, parser=ET.XMLParser())

        try:
            return cls.from_xml_tree(xml_data.getroot())
        except ValueError as e:
            raise ValueError(
                f"Cannot parse {path} as a survey: {e.args[1]}"
            ) from Exception(e.args[1])

    @classmethod
    def from_xml(cls, xml_string: str | bytes) -> Self:
        """Create a Survey from an XML string."""
        xml_data = ET.fromstring(xml_string, parser=ET.XMLParser())
        return cls.from_xml_tree(xml_data)

    @classmethod
    def from_xml_tree(cls, element: ET._Element) -> Self:
        errors = []
        try:
            raw = PlateSurveyXML.from_xml_tree(element)
            return cls(raw)
        ...
        try:
            raw = EchoSurveyReport.from_xml_tree(element)
            return cls(raw)
        except Exception as e:
            errors.append(e)
            pass

        raise ValueError(f"Cannot parse survey: {errors}", errors) from Exception(
            errors
        )
